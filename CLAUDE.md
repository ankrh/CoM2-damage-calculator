...
## Key Files
- `Reference docs/MoM source - Fandom site/` — Primary source for MoM game mechanics. Use these wiki-sourced .md files to determine how combat abilities, damage types, and immunities work.
- `Reference docs/CoM spells.md` — Primary source for CoM spell descriptions and mechanics.
- `Reference docs/CoM2 spells.md` — Primary source for CoM2 spell descriptions and mechanics.
- `Reference docs/CoM2 spells helptext.txt` — Primary source for CoM2 spell and ability descriptions, taken directly from the game UI. Use this to determine exact CoM2 behavior for any spell or ability.
...
## Rules
- When implementing a new mechanic, read the relevant .md files in `Reference docs/MoM source - Fandom site/` first to understand the base MoM rules. For CoM/CoM2-specific behavior, check `Reference docs/CoM spells.md`, `Reference docs/CoM2 spells.md`, `Reference docs/CoM2 spells helptext.txt`.
...
- If already stuck with stale cache, force-reload all scripts via indirect eval:
  ```js
  const scripts = ['Calculator/units.js', 'Calculator/data.js', 'Calculator/engine.js', 'Calculator/combat.js', 'Calculator/stats.js', 'Calculator/ui.js'];
...
- **Reading results**: Use `browser_evaluate` instead of snapshots to read computed values. Key DOM IDs:
  ...
  - `gameVersion`, `cityWalls`, `nodeAura` — global setting dropdowns.
  - `trueLight`, `darkness` — global combat enchantment checkboxes.
  - `aAbil_eternalNight` / `bAbil_eternalNight` — side-owned Eternal Night global enchantment.
